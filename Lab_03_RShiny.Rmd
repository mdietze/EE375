...
Unlike the previous lab, we're going to start with only a small amount of code and have you develop more of the application yourself. Specifically, the code above starts by loading up the met data we've used the last two labs and repeating the (non-interactive) plot of air temperature versus short wave (thermal) radiation. The only difference is that we've thinned the data a bit to make the graphic render more quickly. 

The first thing we're going to do is to add a line to the plot. In the R code block above add the following code after the plot:
```
b0 = 350  ## intercept
b1 = 0    ## slope
...
1. With the intercept, slope, and x in place, next add the calculation of a straight line, and store that in the variable `y`.

2. After calculating y, use `x`,`y` and the `lines` function to add your line to the plot. 

3. Knit your document to verify that the code runs and produces a plot
...
```
This is a pretty unique syntax which we haven't seen before, whereby a whole chunk of code (between the `{` and `}`) is the first arguement to the function. The second argument tells Shiny how big to make the plot. Try knitting your document. Right now renderPlot isn't going to look any different when you Knit it because we haven't made anything interactive yet.
...
```
when we do this we can also drop the lines
```
b0 = 350  ## intercept
b1 = 0    ## slope
```
...
2. Propose a better **metric** that you could use to score whether your fit was any good or not. Implement it in your scoring function, Knit your code, and see if it works (i.e. is maximized or minimized when the fit is 'best'). Describe the rational for your score. Report the best score you could find and the slope and intercept that produce that score.
